...
document.addEventListener('drop', e => {
  ...
  if (file && file.name.endsWith('.rvf')) {
    const reader = new FileReader();
    reader.onload = () => {
      try {
        const newRvf = JSON.parse(reader.result);
        if (newRvf.format === 'rvf' && newRvf.vectors) {
          Object.assign(RVF, newRvf);
          rebuildUI();
          alert('Loaded ' + newRvf.vectors.length + ' vectors from ' + file.name);
        }
      } catch(err) { alert('Invalid .rvf file: ' + err.message); }
    };
...
function rebuildUI() {
  ...
  sel.innerHTML = RVF.vectors.map((v, i) =>
    \`<option value="\${i}">\${v.name} — \${v.style}</option>\`
  ).join('');
  buildSliders();
  search();
}
