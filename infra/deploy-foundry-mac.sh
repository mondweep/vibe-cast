...
if [ -z "$FOUNDRY_ENDPOINT" ]; then
    echo -e "${YELLOW}NEXT STEP: Set Foundry endpoint${NC}"
    echo "  1. Create an Azure AI Foundry hub in the Azure Portal"
    echo "  2. Copy the endpoint URL"
    echo "  3. Update the Function App setting:"
    echo "     az functionapp config appsettings set \\"
    echo "       --resource-group $RESOURCE_GROUP \\"
    echo "       --name $FUNCTION_APP_NAME \\"
    echo "       --settings Foundry__Endpoint=https://your-hub.services.ai.azure.com/"
    echo ""
    echo "  4. Grant the Managed Identity access to Foundry:"
