...
export function PhraseCard({ phrase, respectMode }) {
  ...
  const handleCopy = async () => {
    try {
      await navigator.clipboard.writeText(getKhasiText());
      setCopied(true);
  ...
  return (
    <div className="card hover:border-khasi-accent/50 transition-all duration-200">
      <div className="flex justify-between items-start gap-3">
        <div className="flex-1 min-w-0">
          <p className="text-slate-400 text-sm mb-1">{phrase.english}</p>
          <p className="text-white text-lg font-medium mb-1">{getKhasiText()}</p>
          <p className="text-khasi-accent text-sm italic">{getPhoneticText()}</p>
          {phrase.context && (
