...
function TranslationResult({ result, onCopy }) {
  ...
  const handleCopy = async () => {
    try {
      await navigator.clipboard.writeText(result.khasi);
      setCopied(true);
      onCopy?.();
  ...
  return (
    <div className="card border-khasi-accent/30">
      <div className="flex justify-between items-start gap-3">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-xs px-2 py-0.5 bg-khasi-accent/20 text-khasi-accent rounded-full">
              {result.fromCache ? 'Cached' : 'AI Translation'}
            </span>
          </div>
          <p className="text-slate-400 text-sm mb-1">{result.english}</p>
          <p className="text-white text-lg font-medium mb-1">{result.khasi}</p>
          <p className="text-khasi-accent text-sm italic">{result.phonetic}</p>
        </div>
        ...
        <div className="h-2 bg-slate-700 rounded-full overflow-hidden">
          <div
            className={`h-full transition-all duration-300 ${
              usagePercentage > 90 ? 'bg-red-500' : usagePercentage > 70 ? 'bg-amber-500' : 'bg-khasi-green'
            }`}
