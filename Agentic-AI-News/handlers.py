            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    for r in results:
                        ...
                        # Signal 2: URL contains year patterns → too old
                        if not is_recent:
...
def is_valid_url(url):
    """Check if a URL is real, not a placeholder or fabricated."""
...
def finalize_digest(**kwargs):
    ...
    Strips any items with fabricated or invalid URLs.
    """
    import json
    ...
    # Strip items with invalid URLs
    for category in ["enterprise_adoption", "challenges", "opportunities", "evolving_trends"]:
        items = digest_data.get(category, [])
        if not items:
            continue
        valid = [item for item in items if is_valid_url(item.get("url", ""))]
        removed = len(items) - len(valid)
        if removed:
            print(f"[Validator] Removed {removed} item(s) with invalid URLs from {category}")
        digest_data[category] = valid
