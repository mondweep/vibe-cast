...
create index if not exists chat_analytics_session_id_idx on public.chat_analytics(session_id);
create index if not exists chat_analytics_timestamp_idx on public.chat_analytics(timestamp);

-- Enable RLS
alter table public.chat_analytics enable row level security;

-- Policy: Users can only read analytics for their sessions
create policy "Users can read their analytics" on public.chat_analytics
  for select
  using (auth.uid() = user_id);

create policy "Users can insert analytics for their sessions" on public.chat_analytics
  for insert
