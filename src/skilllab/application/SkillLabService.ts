 ...
 * SkillLabService
 *
 * Application Service for SkillLab domain
 * Responsible for:
 * - Publishing domain events after database transaction commits
 * - Coordinating skill lab workflows
 * - Publishing ExerciseCompleted events
 *
...
export class SkillLabService {
  ...
  async publishEventsFrom(aggregate: AggregateRoot): Promise<void> {
    ...
    this.logger.debug('SkillLabService: publishing domain events', {
    ...
    try {
      ...
      this.logger.info('SkillLabService: all domain events published', {
        aggregateId: aggregate.getId(),
        eventCount: events.length,
      });
    } catch (error) {
      ...
      this.logger.error('SkillLabService: failed to publish domain events', {
        aggregateId: aggregate.getId(),
        error: errorMessage,
      });
  ...
  private async publishEvent(event: DomainEvent): Promise<void> {
    this.logger.debug('SkillLabService: publishing event', {
    ...
    this.logger.debug('SkillLabService: event published', {
      eventName: event.getEventName(),
      eventId: event.id,
      correlationId: event.correlationId,
    });
