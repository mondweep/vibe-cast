...
export const mdxComponents: MDXComponents = {
  ...
  ul: (p) => <ul className="list-disc list-inside space-y-1.5 mb-4 text-muted-foreground" {...p} />,
  ol: (p) => <ol className="list-decimal list-inside space-y-1.5 mb-4 text-muted-foreground" {...p} />,
  ...
  code: ({ className, children, ...p }) => {
    ...
    return isBlock
      ? <code className={`block bg-[#0d1117] text-[#e6edf3] rounded-lg p-4 mb-4 overflow-x-auto text-sm font-mono leading-relaxed ${className ?? ""}`} {...p}>{children}</code>
      : <code className="bg-muted text-primary px-1.5 py-0.5 rounded text-sm font-mono" {...p}>{children}</code>;
  },
  pre: ({ children, ...p }) => <pre className="mb-4 rounded-lg overflow-hidden" {...p}>{children}</pre>,
  table: (p) => (
    <div className="overflow-x-auto mb-6">
      <table className="w-full text-sm border-collapse" {...p} />
    </div>
  ),
  th: (p) => <th className="text-left px-4 py-2 border border-border bg-muted text-foreground font-semibold text-xs uppercase tracking-wider" {...p} />,
  td: (p) => <td className="px-4 py-2.5 border border-border text-muted-foreground" {...p} />,
