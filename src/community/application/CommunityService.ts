 ...
 * CommunityService
 *
 * Application Service for Community domain
 * Responsible for:
 * - Publishing domain events after database transaction commits
 * - Coordinating community workflows
 * - Publishing BadgeEarned and other community events
 *
...
export class CommunityService {
  ...
  async publishEventsFrom(aggregate: AggregateRoot): Promise<void> {
    ...
    this.logger.debug('CommunityService: publishing domain events', {
    ...
    try {
      ...
      this.logger.info('CommunityService: all domain events published', {
        aggregateId: aggregate.getId(),
        eventCount: events.length,
      });
    } catch (error) {
      ...
      this.logger.error('CommunityService: failed to publish domain events', {
        aggregateId: aggregate.getId(),
        error: errorMessage,
      });
  ...
  private async publishEvent(event: DomainEvent): Promise<void> {
    this.logger.debug('CommunityService: publishing event', {
    ...
    this.logger.debug('CommunityService: event published', {
      eventName: event.getEventName(),
      eventId: event.id,
      correlationId: event.correlationId,
    });
