...
- **Date:** 2026-08-06
- **Decision history**, kept because it is the useful part of this record:
  proposed as below → **overruled** in favour of Supabase (project
  `aws-advanced-networking`, everything namespaced `floodmonitoring_`) → **the
  overrule withdrawn** on the grounds that nothing built needs a backend, and a
  backend should not be adopted before something requires it. The intermediate
  version was written and is in the git history at `6c186f7`; if Supabase is
  revisited, start from that file rather than from scratch — the schema sketch,
  the `floodmonitoring_` prefixing rule and the RLS questions in it are all
...
The model was built before this was decided, which turned out to be the useful
experiment: `integrateOverPeriod` and the person-day integrals are arithmetic
over data already in memory, and at no point did a backend's absence make
anything harder. A dependency that nothing needs is not a foundation, it is a
liability taken on early.

The question to revisit this on is not "is Supabase good" — it is fine — but
**"what can we not do without it?"** Today the answer is: share a plan with a
colleague, and hold a bulletin published after the last deploy. Neither is
blocking, and neither is what the economic model is for. When one of them starts
costing real friction, reopen this with that friction as the evidence.
