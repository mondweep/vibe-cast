    ...
    <div className="relative w-full max-w-md">
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 px-4 py-3 text-lg border-2 border-gray-200 rounded-xl focus:border-indigo-500 focus:outline-none transition-colors bg-white"
          autoComplete="off"
          spellCheck={false}
        />
        <button
      ...
      {showSuggestions && (
        <ul className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-60 overflow-auto">
          {suggestions.map((name, i) => (
            <li key={i}>
              <button
                className="w-full text-left px-4 py-2 hover:bg-indigo-50 transition-colors first:rounded-t-xl last:rounded-b-xl"
                onMouseDown={(e) => {
                  e.preventDefault();
                  setInput(name);
                  handleSubmit(name);
              ...
              >
                ...
                <span className="text-gray-600">{name.substring(input.length)}</span>
              </button>
            </li>
          ))}
        </ul>
